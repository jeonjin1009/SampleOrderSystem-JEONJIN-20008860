...
| 케이스 | 승인(approveOrder) 시 | 생산 완료(ProductionLine) 시 | 출고(release) 시 |
|---|---|---|---|
| 재고 충분 | 차감 없음 (상태만 CONFIRMED) | — | 재고 -= quantity ✅ |
| 재고 부족 | 차감 없음 | 재고 += requiredQty | 재고 -= quantity ✅ |

→ **`approveOrder` 수정**: 재고 충분 케이스의 재고 차감 로직 제거.
→ **`release()` 수정**: SampleRepository 주입 추가, 항상 `재고 -= quantity` 차감.
→ **`OrderControllerTest.approveOrder_재고충분_재고가차감된다` 수정**: "승인 시 재고 불변" 검증으로 변경.
...
| 출고 시 재고 차감 | **출고(release) 시 항상 재고 -= quantity** / approveOrder 충분 케이스의 재고 차감 제거 (버그 수정) |
...
### 7-3. OrderControllerTest 수정 (버그 수정)

| 테스트 메서드 | 변경 내용 |
|---|---|
| `approveOrder_재고충분_재고가차감된다` → `approveOrder_재고충분_재고불변` | 승인 후 재고가 변하지 않음을 검증 (차감 시점이 release로 이동) |

### 7-4. ReleaseControllerTest 추가 (버그 수정 검증)

| 테스트 메서드 | 검증 내용 |
|---|---|
| `release_재고충분케이스_재고차감확인` | CONFIRMED 주문 출고 시 재고 -= quantity 확인 |
| `release_재고부족케이스_재고차감확인` | 생산 완료(재고 += requiredQty) 후 release 시 재고 -= quantity 확인 |

### 7-5. MonitoringControllerTest 추가 (버그 수정 검증)

| 테스트 메서드 | 검증 내용 |
|---|---|
| `getStockStatus_CONFIRMED_RELEASE_주문_제외` | CONFIRMED/RELEASE 주문은 재고 상태 판단 시 주문수량 합산에서 제외됨 확인 |

### 7-6. AppIntegrationTest 추가
...
```
[버그 수정 — RED/GREEN]
1. OrderControllerTest 수정: approveOrder_재고충분_재고불변 (RED → 기존 테스트가 새 동작으로 실패)
2. ReleaseControllerTest 추가: release 재고 차감 2개 (RED)
3. MonitoringControllerTest 추가: CONFIRMED/RELEASE 제외 기준 (RED)
4. OrderController.approveOrder() 재고 충분 케이스 차감 제거 (GREEN)
5. ReleaseController: SampleRepository 주입 + release() 재고 차감 추가 (GREEN)
6. MonitoringController.getStockStatus() 주문수량 기준 수정 (GREEN)

[View 신규 구현 — RED/GREEN]
7. MonitoringView 시그니처 + MonitoringViewTest 작성 (RED)
8. ReleaseView 시그니처 + ReleaseViewTest 작성 (RED)
9. AppIntegrationTest 추가 (RED)
10. MonitoringView 구현 (GREEN)
11. ReleaseView 구현 (GREEN)
12. MainView 메뉴 4번(모니터링), 6번(출고 처리) 활성화
13. App.java: MonitoringController, ReleaseController, View 인스턴스 생성 및 연결
14. 수동 실행으로 전체 흐름 확인
```
